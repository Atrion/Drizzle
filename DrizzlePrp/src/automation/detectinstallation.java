...
public class detectinstallation
{
    ...
    public static boolean isFolderMoul(String moulfolder)
    {
        File moulfile = new File(moulfolder);
        if(!moulfile.exists())
        {
            m.err("The MOUL folder you selected doesn't exist.");
            return false;
        }
        if(!moulfile.isDirectory())
        {
            m.err("The MOUL folder you selected must be a folder, not a file.");
            return false;
        }
        File moulexe = new File(moulfolder+"/tos.txt");
        if(!moulexe.exists())
        {
    ...
    public static boolean isFolderMyst5(String myst5folder)
    {
